...
{
    CU_ASSERT (_run_lua ("apteryx = require('apteryx-xml')"));
...
test_lua_load_api_memory (void)
{
    ...
    L = luaL_newstate ();
    luaL_openlibs (L);
    res = luaL_loadstring (L, "apteryx = require('apteryx-xml').api('"TEST_SCHEMA_PATH"')");
    if (res == 0)
        res = lua_pcall (L, 0, 0, 0);
    if (res != 0)
        fprintf (stderr, "%s\n", lua_tostring(L, -1));
...
test_lua_load_api_performance (void)
{
    ...
    for (i = 0; i < 10; i++)
    {
        CU_ASSERT (_run_lua ("apteryx = require('apteryx-xml').api('"TEST_SCHEMA_PATH"')"));
    }
...
test_lua_api_perf_get ()
{
    ...
    L = luaL_newstate ();
    luaL_openlibs (L);
    CU_ASSERT (luaL_loadstring (L, "apteryx = require('apteryx-xml').api('"TEST_SCHEMA_PATH"')") == 0);
...
static CU_SuiteInfo suites[] = {
    { "LUA API", suite_init, suite_clean, tests_lua },
    CU_SUITE_INFO_NULL,
};
